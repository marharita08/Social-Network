...
const ArticleContainer = ({setArticleContext, article, handleError, articles, setArticles}) => {
    ...
    const {isFetching:commentsAmountFetching } = useQuery(`comments amount ${id}`, () => getCommentsAmount(id), {
        onSuccess: (data) => setCommentsAmount(parseInt(data?.data, 10))

    });
    const {isFetching:likesAmountFetching } = useQuery(`likes amount ${id}`, () => getLikesAmount(id), {
        onSuccess: (data) => setLikes(parseInt(data?.data, 10))
    })
    ...
    const deleteCommentFromArray = (id) => {
        ...
        const count = newCommentsArray.filter(obj => obj.path.startsWith(id + ".")
            || obj.path.includes("." + id + ".")).length;
        newCommentsArray.splice(index, count + 1);
        setCommentsArray(newCommentsArray);
        setCommentsAmount(commentsAmount - (count + 1));
    }

    const handleLikeClick = (event) => {
        event.preventDefault();
        if (!isLiked) {
            addLikeMutate({article_id: article.article_id, user_id});
        } else {
                ...
                <div className="inner">
                    ...
                    <Collapse in={commentsExpanded} timeout="auto" unmountOnExit>
                        {commentsArray?.map((comment) =>
                            <ErrorBoundary key={"Comment " + comment.comment_id}>
                                <CommentContainer
                                    comment={comment}
